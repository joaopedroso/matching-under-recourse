...
Results go to `RESULTS/solve-<interpreter>-<date>-<host>/`.  
Process and plot:
...
`RESULTS_2024-03-22/` and `RESULTS_2025-01-13/` were produced with an older
encoding (N = 0, 1, 2, 3, 999999 corresponding to paper N = 1, 2, 3, 4, ∞)
and are therefore **not** directly compatible with the current plotting scripts.
To regenerate the summary CSVs in the new encoding, re-run the experiment
scripts and process with `process_output.sh`.
